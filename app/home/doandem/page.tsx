...
export default function DoAnDemPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const filtered = data.filter((item: Product) => item.danhMucId === 2 && item.trangThai === "con_hang");
        setProducts(filtered);
      } catch (error) {
        console.error("Lỗi lấy sản phẩm đồ ăn đêm:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="category-page">
      ...
      <div className="category-page-header">
        <h1>🌙 Đồ Ăn Đêm</h1>
        <p>Thưởng thức những món ăn đêm nóng hổi, giao hàng tận nơi cho đêm khuya</p>
      </div>
      ...
      <div className="products-4col">
        {loading ? (
          ...
          <p>Chưa có sản phẩm đồ ăn đêm.</p>
        ) : (
